class EventsController < ApplicationController
  ...
  def index
    ...
    if current_user.babies.ids.include?(baby_id)
      ...
      # logic prevents page from breaking when no profile pic preset
      ...
      @media = @events.map do |event|
        # logic prevents page from breaking when no event media preset
        if event.media.any?
      ...
      # define front-end variables for JS with gon gem
      gon.baby = @baby
